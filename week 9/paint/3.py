...
while True:
    ...
    for event in pygame.event.get():
        ...
        if tool == 'rect': 
            figurs.draw_rectangle(event, screen, another_layer, color)

        if tool == 'circle': 
            figurs.draw_circle(event, screen, another_layer, color)

        if tool == 'triangle': 
            figurs.draw_triangle(event, screen, another_layer, color)

        if tool == 'equilateral triangle': 
            figurs.draw_equilateral_triangle(event, screen, another_layer, color)

        if tool == 'rhombus': 
            figurs.draw_rhombus(event, screen, another_layer, color)

        if tool == 'erasor':
            figurs.erasor(event, screen, another_layer)
